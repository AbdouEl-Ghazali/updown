...
const Calculator = ({metrics}: any) => {
    ...
    const calculate = async(funds:number, fees:number) => {
        ...
        for (var key in price) {
            if (Object.keys(price).indexOf(key) != (Object.keys(price).length)-1){ // Don't include the last entry
                ...
                const percentChange: any = Math.abs((currentPrice-nextPrice)/currentPrice)
                const lastForecast: any = Math.round(forecasted[key])
  ...
  return (
    <div className='flex flex-auto w-full justify-center'>
        <div className={`flex flex-col lg:flex-row sm:gap-5 p-2 sm:p-5 w-full bg-zinc-200 bg-opacity-50 dark:bg-zinc-700 dark:bg-opacity-50 rounded-xl`}>
